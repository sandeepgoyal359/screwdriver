 ...
 * @param  {Function} callback
 */
function registerJSONAPIPlugins(server, config, callback) {
    const plugins = [
        'common',
        // 'auth',
        'builds',
    ...
    async.eachSeries(plugins, (pluginName, next) => {
        server.register({
            register: require(`../plugins/jsonapi/${pluginName}`),
            options: config[pluginName] || {}
        }, {
            routes: {
                prefix: '/v5'
...
/**
 * [registerJSONAPIPlugins2 description]
 * @method registerJSONAPIPlugins2
 * @param  {[type]}                server   [description]
 * @param  {[type]}                config   [description]
 * @param  {Function}              callback [description]
 * @return {[type]}                         [description]
 */
function registerJSONAPIPlugins2(server, config, callback) {
    const plugins = [
        // 'common',
        // 'auth',
        'builds',
    ...
    async.eachSeries(plugins, (pluginName, next) => {
        server.register({
            register: require(`../plugins/jsonapi2/${pluginName}`),
            options: config[pluginName] || {}
        }, {
            routes: {
                prefix: '/v6'
...
module.exports = (server, config) => (
    new Promise((resolve, reject) => {
        async.series([
            ...
            async.apply(registerJSONAPIPlugins, server, config),
            async.apply(registerJSONAPIPlugins2, server, config)
        ], (err) => {
            if (err) {
                return reject(err);
            }
